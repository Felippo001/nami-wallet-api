...
function NamiWalletApi(NamiWalletObject, blockfrostApiKey, serializationLib) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        function send(_a) {
            ...
            return __awaiter(this, void 0, void 0, function () {
                var lovelace, ReceiveAddress, PaymentAddress, protocolParameter, utxos, AssetsMap, _i, assets_1, asset, _f, policy, assetName, quantity, multiAsset, policy, ScriptHash, Assets, _assets, _g, _assets_1, asset, AssetName, BigNum, outputValue, minAda, outputs, RawTransaction;
                return __generator(this, function (_h) {
                    switch (_h.label) {
                        case 0:
                        ...
                        case 1:
                            PaymentAddress = _h.sent();
                            return [4 /*yield*/, _getProtocolParameter()];
                        case 2:
                            protocolParameter = _h.sent();
                            return [4 /*yield*/, getUtxosHex()];
                        case 3:
                            utxos = (_h.sent()).map(function (u) { return S.TransactionUnspentOutput.from_bytes(Buffer.from(u, 'hex')); });
                            AssetsMap = {};
                            for (_i = 0, assets_1 = assets; _i < assets_1.length; _i++) {
                                asset = assets_1[_i];
                                _f = asset.unit.split('.'), policy = _f[0], assetName = _f[1];
                            ...
                            multiAsset = S.MultiAsset.new();
                            for (policy in AssetsMap) {
                                ScriptHash = S.ScriptHash.from_bytes(Buffer.from(policy, 'hex'));
                                Assets = S.Assets.new();
                                _assets = AssetsMap[policy];
                                for (_g = 0, _assets_1 = _assets; _g < _assets_1.length; _g++) {
                                    asset = _assets_1[_g];
                                    AssetName = S.AssetName.new(Buffer.from(asset.unit, 'hex'));
                                    BigNum = S.BigNum.from_str(asset.quantity);
                                    Assets.insert(AssetName, BigNum);
                                }
                                multiAsset.insert(ScriptHash, Assets);
                            }
                            outputValue = S.Value.new(S.BigNum.from_str(lovelace));
                            minAda = S.min_ada_required(outputValue, S.BigNum.from_str(protocolParameter.minUtxo));
        ...
        function _txBuilder(_a) {
            ...
            if (changeMultiAssets && change.to_bytes().length * 2 > VALUE_SIZE) {
                ...
                var minAda = S.min_ada_required(partialChange, ProtocolParameter.minUtxo);
